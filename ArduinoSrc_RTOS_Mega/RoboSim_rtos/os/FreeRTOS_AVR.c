...
void vApplicationStackOverflowHook(TaskHandle_t pxTask, char *pcTaskName) {
    ...
    char temp_string[80];
    const char * msg_hdr = "OSERR:\nStack Overflow\nTask: ";
    
    strncat(temp_string, msg_hdr, sizeof(msg_hdr));
    strncat(temp_string, pcTaskName, sizeof(pcTaskName));
    assertMsg(temp_string, 0);
	errorBlink(3);
}
